...
{
    internal class adminCustomerClass: com
    {
        public adminCustomerClass() {}
        com c = new com();
        ...
        public adminCustomerClass(adminCustomerForm CustomerForm)
        {
               ...
               try
                {
                    if (!c.checkCustomer(_uname.Text))
                    {
                        ...
                        c.ExecuteQuery(addCustomer, queryType.save);
                        viewGrid();
                        clearFields();
            ...
            else
            {
                if(MessageBox.Show("Are you sure want to Update =>" + "Customer ID: " + _idCombo.Text.Trim() + " ?", "Update Confirmation", MessageBoxButtons.YesNo, MessageBoxIcon.Warning)==DialogResult.Yes)
                {
                    try
                    {
                        ...
                        c.ExecuteQuery(updateCustomer, queryType.update);
                        viewGrid();
                        clearFields();
        ...
        public void deleteCus()
        {
            if (MessageBox.Show("Are you sure want to Delete =>" + "Customer ID: " + _idCombo.Text.Trim() + " ?", "Delete Confirmation", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == DialogResult.Yes)
            {
                try
                {
                    ...
                    c.ExecuteQuery(deleteCustomer, queryType.delete);
                    viewGrid();
                    clearFields();
        ...
        {
            c.loadTableDataAsFK("Select * from Customer", _idCombo,"cusID","cusID");
        }

        public void cusImageImport()
        {
            c.importImage(_picture);
        }
